...
```{r}
sw_split_name <- starwars %>% # create new subset from starwars data
  separate(col = name,
           into = c("name_1","name_2"), # split name into 2 columns
           sep = "-" #separated at dash
  )
```
...
sw_skin_sep <- starwars %>% # start from starwars data
  separate(col = skin_color,
           into = c("skin_1","skin_2"), # split skin color into 2 columns
           sep = ",", # separated at comma
           extra = "merge", # merge into second column if additional commas
...
```{r}
some_dates <- c(04251997, 07042001, 04152008)
...
gasol_hookshots <- lakers %>% 
  ...
  mutate(month = month(date_updated)) %>% 
  count(month)
